...
def main():
    ...
    try:
        ...
        print(f"Scaler not found or error loading: {e}. Train the model first using rl_train.py.")
        return
    ...
    try:
        ...
        print(f"Model not found or error loading: {e}. Train the model first using rl_train.py.")
        return
        
    # Create test env
    env = DummyVecEnv([lambda: TradingEnv(test_df, WINDOW_SIZE, TP_MULTIPLIER, SL_MULTIPLIER)])
    obs = env.reset()
    ...
    plt.title(f"RL Agent (PPO) Backtest on {SYMBOL} (D1)")
