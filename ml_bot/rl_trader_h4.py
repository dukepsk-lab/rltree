...
from rl_train_h4 import fetch_data, add_features

# NOTE: rl_model_h4.zip was trained on the legacy TradingEnv (long-only,
...
LOT_SIZE = 0.01
WINDOW_SIZE = 20
TP_PRICE_DIFF = 3.00   # must match TP_PRICE_DIFF in rl_train_h4.py
...
def wait_for_new_bar(current_bar_time):
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Waiting for the next H4 bar to open...")
    while True:
...
def main():
    ...
    try:
        model = PPO.load(os.path.join(BASE_DIR, "rl_model_h4"))
        scaler = joblib.load(os.path.join(BASE_DIR, "rl_scaler_h4.save"))
