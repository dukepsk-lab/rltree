...
def main():
    ...
    # Load Scaler
    try:
        scaler = joblib.load('ml_bot/rl_scaler_h8.save')
    except Exception as e:
        print(f"Scaler not found or error loading: {e}. Train the model first using rl_train_h8.py.")
        return
    ...
    # Load Model
    try:
        model = PPO.load("ml_bot/rl_model_h8")
    except Exception as e:
        print(f"Model not found or error loading: {e}. Train the model first using rl_train_h8.py.")
        return
    ...
    plt.title(f"RL Agent (PPO) Backtest on {SYMBOL} (H8)")
